        ...
        <div class="max-h-screen w-11/12 mx-auto sm:mx-0 sm:w-80 border rounded shadow block px-3 py-3 mb-5 sm:mb-0">
            ...
            <div id="filter" class="space-y-4 hidden sm:block">
                ...
                <div v-for="country in countries" :key="country">
                    <router-link :to="'/packages/' + country.slug"
                    ...
                    <div v-show="country.slug == active_country" class="px-2  ">
                        <div class="button space-y-3" v-for="destination in country['items']" :key="destination">
                ...
                <div class="h-80 overflow-y-scroll space-y-3">
                    ...
                    <div class="button text-center" v-for="category in categories" :key="category">
...
const { getFilteredPacks, packages, alter_pages, getDestinationPacks } = usePackages();
...
watch(filter.value, () => {
    if (filter.value.destination == '*' && filter.value.category == '*')
        getDestinationPacks(currentPage.value, props.destination)
    else getFilteredPacks(filter.value)
})
